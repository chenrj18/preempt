...
public class PreemptA {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("Average response time for high priority in type A: " + highresponse / numhigh);
        for (int ii = 0; ii < numlow; ii++) {
            lowresponse = lowresponse + finishedlow.peek().getResponse();
            finishedlow.add(finishedlow.poll());
        }
        System.out.println("Average response time for low priority in type A: " + lowresponse / numlow);
        for (int ii = 0; ii < numhigh; ii++) {
            totalhighburstcompletion = totalhighburstcompletion + finishedhigh.poll().getburstCompletion();
        }
        System.out.println("Average burst-completion time for high priority in type A: " + totalhighburstcompletion / numhigh);
        for (int ii = 0; ii < numlow; ii++) {
            totallowburstcompletion = totallowburstcompletion + finishedlow.poll().getburstCompletion();
        }
        System.out.println("Average burst-completion time for low priority in type A: " + totallowburstcompletion / numlow);
        System.out.println("% of time doing context switching for type A: " + (totalswitch / (double) timer) * 100 + "%");
    }
}
